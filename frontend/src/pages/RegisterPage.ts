...
export function createRegisterPage(): HTMLElement {
  ...
  registerCard.className = 'bg-white rounded-lg shadow-sm p-6 w-full max-w-sm';

  registerCard.innerHTML = `
    <div class="text-center mb-6">
      <h1 class="text-xl font-semibold text-gray-800 mb-1">Register</h1>
      <p class="text-gray-500 text-sm">Join the game</p>
    </div>
    ...
    <form id="registerForm" class="space-y-4">
      ...
      <div>
        ...
        <input 
          ...
          placeholder="••••••••"
        >
        <p class="text-xs text-gray-400 mt-1">Min 8 chars</p>
      </div>

      <div>
        <label for="confirmPassword" class="block text-xs font-medium text-gray-600 mb-1">Confirm</label>
        <input 
          type="password" 
          id="confirmPassword" 
    ...
    <div class="mt-4 text-center">
      <p class="text-gray-500 text-sm">
        Have account? 
        <a href="#" id="goToLogin" class="text-blue-500 hover:text-blue-600">Sign in</a>
      </p>
    </div>
    ...
    <div id="loadingSpinner" class="mt-3 text-center hidden">
      <span class="text-gray-500 text-sm">Creating account...</span>
    </div>
  `;

  // Event listeners
  const form = registerCard.querySelector('#registerForm') as HTMLFormElement;
  const goToLoginBtn = registerCard.querySelector('#goToLogin') as HTMLAnchorElement;
  const googleRegisterBtn = registerCard.querySelector('#googleRegister') as HTMLButtonElement;
  const errorDiv = registerCard.querySelector('#errorMessage') as HTMLDivElement;
  const loadingDiv = registerCard.querySelector('#loadingSpinner') as HTMLDivElement;

  // Password confirmation validation
  const passwordInput = registerCard.querySelector('#password') as HTMLInputElement;
  const confirmPasswordInput = registerCard.querySelector('#confirmPassword') as HTMLInputElement;
